...
class CameraNode(object):
    ...
    def kmeans_algo(self, stream_data):
        ...
        error, cluster_points, centroids = self.kmeans_implementation(Z, K, 10)
    ...
    def assign_pixels(self, cluster_centroid_values, pixel_values):
        num_of_clusters, _ = np.shape(cluster_centroid_values)
        distance_list = []
        for i in range(num_of_clusters):
            cluster_centroid = cluster_centroid_values[i]
            #getting the distance between the cluster_centroid_value 
            distance = math.sqrt(((cluster_centroid[0] - pixel_values[0])**2)+((cluster_centroid[1] - pixel_values[1])**2)+((cluster_centroid[2] - pixel_values[2])**2))
            distance_list.append(distance)
        index, value = min(enumerate(distance_list), key=operator.itemgetter(1))
        return index, value

    def find_centroids(self, clusters_points, Z, number_of_channels, old_centroids):
        new_centroids = np.zeros(shape=(len(clusters_points), number_of_channels))
        for i in range(len(clusters_points)):
            cluster_points_indices = clusters_points[i]
            if len(cluster_points_indices) == 0:
                new_centroids[i, :] = old_centroids[i, :]
                continue
            cluster_points_values = Z[cluster_points_indices, :]
            mean = np.mean(cluster_points_values, 0)
            mean = np.ravel(mean)
            new_centroids[i, :] = mean
        return new_centroids
        
    def kmeans_implementation(self, Z, K, num_of_iterations):
        #Z is the image
        num_pixels, number_of_channels = np.shape(Z)
        centroids = np.random.randint(0, 255, size=(K, number_of_channels))
        error = np.zeros(shape=(num_pixels, 1))
        cluster_points = []
        for iteration in range(num_of_iterations):
            ...
            #assigning each pixel
            for i in range(num_pixels):
                pixel_values = Z[i, :]
                cluster_index, ssd = self.assign_pixels(centroids, pixel_values)
                error[i] = ssd
                cluster_points[cluster_index].append(i)
            centroids= self.find_centroids(cluster_points, Z, number_of_channels, centroids)

        pixel_assignments = np.zeros(shape=(num_pixels, 1), dtype=np.int32)
        for i in range(K):
            pixel_assignments[cluster_points[i]] = i
        return error, pixel_assignments, centroids # reverse cluster_points
